...
tags: [Coffee]
draft: true
summaryText: <p>Last week, the United States Department of Agriculture (USDA) released their biannual estimate of global supply and demand of coffee. This estimate contains a forecast for the upcoming 2017/18 coffee year, which starts in October, giving us an indication of where the coffee sector might be up to September 2018.<p>Obviously, much can change in that time, particularly as weather events become more unpredictable, but there are still some useful morsels to be found in the data.<p><img src="/img/coffee-stocks.png" alt="a stock photo of some coffee stocks" title="a stock photo of coffee stocks"/><p>
---
...
Last week, the United States Department of Agriculture (USDA) released their biannual estimate of global supply and demand of coffee. This estimate contains a forecast for the upcoming 2017/18 coffee year, which starts in October, giving us an indication of where the coffee sector might be going up to September 2018. 
...
```{r, echo = FALSE}
htmltools::HTML('<p class = "message">There are two main sources of publicly available statistics for the coffee sector: the USDA and the ICO (International Coffee Organization). The USDA is generally known for being on the higher side of production estimates, while the ICO is more conservative in its outlook and doesn\'t tend to release production forecasts. The truth, as usual, is probably somewhere in between. </p>')
```
...
In relative terms, although the USA and the European Union (as a bloc) have both added around 3.5 million bags to their overall coffee consumption, this isn't a massive increase on their previous levels. The increases in East & Southeast Asia, on the other hand, suggest rapidly expanding markets. In fact the largest percentage increases in both production and consumption have come from *China*, although the accuracy of these figures is debatable. 
...
plot_ly() %>% 
    add_trace(name = "Stocks/use ratio",
        ...
        hoverinfo = "text",
        marker = list(color = blogColours[1], opacity = 0.5)) %>% 
