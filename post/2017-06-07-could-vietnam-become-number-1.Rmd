...
plot_ly() %>%
    ...
    add_ribbons(data = filter(splitLoess, country == "Vietnam"),
               ...
               ymin = ~.fitted - 1.96*.se.fit,
                ymax = ~.fitted + 1.96*.se.fit) %>%
    add_annotations(x = 1.15, y = -0.22, 
                    xref = "paper", yref = "paper", 
                    showarrow = F,
                    xanchor = "right", align = "right",
                    text = "<i>Sources: UN Comtrade, General\nDepartment of Vietnam Customs, ICO</i>") %>%
...
pBr <- plot_ly(prodAll %>%
    ...
    add_annotations(text = "Brazil", font = list(family = "sans-serif", color = "#515151"),
...
pVn <- plot_ly(prodAll %>%
    ...
    add_annotations(text = "Vietnam", font = list(family = "sans-serif", color = "#515151"),
                    bgcolor = "#D9D9D9", borderpad = 4, showarrow = FALSE, width = 120,
                    xref = "paper", x = 0.5, xanchor = "center",
                    yref = "y", y = 62, yanchor = "top") %>%
    layout(yaxis = list(zeroline = FALSE))
