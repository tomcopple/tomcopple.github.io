...
allComsRaw <- read_csv(file.path(basedir, "output", "allCommodities.csv")) %>% 
    filter(!(commodity %in% c("Soya-bean oil", "Barley", "Olive oil", "Uranium")))
```
...
allComsClean <- allComsRaw %>% 
    mutate(country = case_when(
        ...
        TRUE ~ .$country),
        commodity = ifelse(grepl("oil", commodity), "Oil", commodity)
    ) %>% 
...
allComsDev <- allComsClean%>% 
    ...
    ungroup() %>% 
    filter(commodity %in% c("Coal", "Coffee", "Copper", "Gold", "Maize", "Natural gas", "Palm oil", "Oil", "Soya beans", "Sugar", "Timber", "Wheat")) %>% 
    mutate(commodity = ordered(commodity, levels = rev(commodity)))
...
p1 <- allComsDev %>% 
    ...
    layout(title = "Commodity exports by income group",
           annotations = list(x = 0, y = 11, text = "Low-income", showarrow = FALSE,
                              align = "left", xref = "paper"),
           yaxis = list(title = "", showline = FALSE, zeroline = FALSE),
...
p2 <- allComsDev %>% 
    ...
    layout(annotations = list(x = 0, y = 11, text = "Lower-middle-income", showarrow = FALSE,  xref = "paper", xanchor = "left"),
           yaxis = list(title = "", showline = FALSE, zeroline = FALSE),
...
p3 <- allComsDev %>% 
    ...
    layout(annotations = list(x = 0, y = 11, text = "Upper-middle-income", showarrow = FALSE, xanchor = "left", xref = "paper"),
           yaxis = list(title = "", showline = FALSE, zeroline = FALSE),
           xaxis = list(title = "", showline = FALSE, zeroline = FALSE),
           margin = list(l = 0),
           showlegend = FALSE,
